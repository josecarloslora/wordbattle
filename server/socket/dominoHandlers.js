...
function getPlayersInfo(room) {
  return Array.from(room.players.entries()).map(([id, p]) => ({
    id, username: p.username, avatarColor: p.avatarColor,
    team: p.team, seat: p.seat, tileCount: p.hand.length,
    isHost: p.isHost, isReady: p.isReady, isBot: p.isBot || false,
  }));
}
...
function doBotMove(io, roomCode, room) {
  ...
  if (validTiles.length === 0) {
    room.passCount++;
    io.to(roomCode).emit('domino:player-passed', { playerId: botId, username: bot.username, passCount: room.passCount });
    if (room.passCount >= 4) { endRound(io, roomCode, room, null, true); return; }
  ...
  io.to(roomCode).emit('domino:tile-played', {
    playerId: botId, tileId: tile.id, side,
    board: room.board, leftEnd: room.leftEnd, rightEnd: room.rightEnd,
    players: getPlayersInfo(room),
  });

  if (bot.hand.length === 0) { endRound(io, roomCode, room, botId, false); return; }
  advanceTurn(io, roomCode, room);
}
...
// triggerSocket: if provided, emit directly to it AND to room (belt-and-suspenders)
async function startNewRound(io, roomCode, room, isFirstRound, triggerSocket) {
  try {
    const tiles = shuffle(generateAllTiles());
    const seats = getSeatOrder(room);
    console.log(`[domino] startNewRound ${roomCode} seats=${seats.length} isFirst=${isFirstRound}`);

    for (let i = 0; i < seats.length; i++) {
      const player = room.players.get(seats[i]);
      if (!player) { console.error(`[domino] missing player at seat index ${i}: ${seats[i]}`); continue; }
    ...
    // Round 1: whoever has 6-6 must start (and must play it first)
    ...
    if (isFirstRound) {
      ...
      starterId = room.lastWinner;
    }
    if (!starterId) starterId = seats[0];
    room.currentPlayer = starterId;
    room.forcedFirstTile = isFirstRound ? '6-6' : null;

    console.log(`[domino] Round ${room.roundNumber} starter=${room.players.get(starterId)?.username}`);
    ...
    if (isFirstRound && !room.gameId) {
      try {
        ...
        console.error('[domino] insert domino_games error:', dbErr.message);
      }
    }
    ...
    // Send personalized round-starting (with hand + myTeam) to each human player
    for (const [uid, player] of room.players) {
      if (!player.isBot && player.socketId) {
        const personalPayload = { ...payload, hand: player.hand, myTeam: player.team };
        console.log(`[domino] Personal round-starting to ${player.username} (${player.socketId})`);
        io.to(player.socketId).emit('domino:round-starting', personalPayload);
      }
    }

    // Also broadcast to room (catches any socket not in players map yet, e.g. fresh reconnect)
    console.log(`[domino] Room broadcast round-starting to ${roomCode}`);
    io.to(roomCode).emit('domino:round-starting', payload);

    // Legacy: keep hand-update for compatibility
    for (const [uid, player] of room.players) {
      if (!player.isBot && player.socketId) {
        io.to(player.socketId).emit('domino:hand-update', { hand: player.hand });
      }
    }
    ...
    console.error('[domino] startNewRound CRASH:', err.message, err.stack);
  }
}
...
async function endRound(io, roomCode, room, winnerId, isTranque) {
  ...
  if (winnerTeam !== null) room.scores[`team${winnerTeam}`] += totalPoints;
  ...
  } catch (err) { console.error('[domino] endRound DB err:', err.message); }

  const gameOver = room.scores.team1 >= room.targetScore || room.scores.team2 >= room.targetScore;
  const roundResult = {
    winnerTeam, points: totalPoints, isTranque,
    scores: { ...room.scores }, players: getPlayersInfo(room),
  };
  ...
  if (gameOver) {
    ...
    try {
      if (room.gameId) await query('UPDATE domino_games SET ended_at=NOW(), winner_team=$1 WHERE id=$2', [gameWinnerTeam, room.gameId]);
      await query("UPDATE rooms SET status='waiting' WHERE code=$1", [roomCode]);
    } catch (err) { console.error('[domino] game over DB err:', err.message); }
    io.to(roomCode).emit('domino:game-over', { ...roundResult, gameWinnerTeam });
  } else {
    room.roundNumber++;
    io.to(roomCode).emit('domino:round-over', roundResult);
    setTimeout(() => startNewRound(io, roomCode, room, false, null), 5000);
  }
}
...
function register(io, socket, activeDominoRooms) {
  socket.on('domino:join-room', async ({ roomCode }) => {
    try {
      ...
      if (roomRow.game_type && roomRow.game_type !== 'domino') {
        return socket.emit('domino:error', { message: 'Esta no es una sala de dominó' });
      }

      socket.join(roomCode);
      console.log(`[domino] ${socket.user.username} joined socket room ${roomCode}`);

      if (!activeDominoRooms.has(roomCode)) {
        activeDominoRooms.set(roomCode, {
          players: new Map(), board: [], leftEnd: null, rightEnd: null,
          currentPlayer: null, passCount: 0, roundNumber: 1,
          scores: { team1: 0, team2: 0 }, status: 'waiting', readyCount: 0,
          lastWinner: null, gameId: null, targetScore: 200, forcedFirstTile: null,
        });
      }
      ...
      if (dr.players.has(socket.user.id)) {
        ...
        console.log(`[domino] ${socket.user.username} reconnected to ${roomCode}, status=${dr.status}`);

        if (dr.status === 'playing') {
          socket.emit('domino:game-state', {
            hand: player.hand,
            board: dr.board, leftEnd: dr.leftEnd, rightEnd: dr.rightEnd,
            currentPlayer: dr.currentPlayer, players: getPlayersInfo(dr),
            scores: { ...dr.scores }, roundNumber: dr.roundNumber,
            myTeam: player.team, forcedFirstTile: dr.forcedFirstTile,
          });
        } else {
          socket.emit('domino:room-state', {
            players: getPlayersInfo(dr), status: dr.status,
            scores: { ...dr.scores }, roundNumber: dr.roundNumber,
          });
        }
        return;
      ...
      if (humanCount >= 4) return socket.emit('domino:error', { message: 'Sala llena' });
      ...
      try {
        await query(
          ...
          [roomRow.id, socket.user.id]
        );
      } catch (dbErr) {
        console.error('[domino] room_members insert err:', dbErr.message);
      }

      const team = [1, 2, 1, 2][seatIndex];
      dr.players.set(socket.user.id, {
        socketId: socket.id, username: socket.user.username, avatarColor: '#6366f1',
        team, seat: seatIndex, hand: [], isReady: false,
        isHost: roomRow.host_id === socket.user.id, isBot: false,
      });
      ...
      socket.to(roomCode).emit('domino:player-joined', {
        userId: socket.user.id, username: socket.user.username, players: getPlayersInfo(dr),
      });
      console.log(`[domino] ${socket.user.username} joined ${roomCode} seat=${seatIndex} team=${team}`);
    } catch (err) {
      console.error('[domino:join-room] ERROR:', err.message, err.stack);
  ...
  socket.on('domino:ready', async ({ roomCode }) => {
    try {
      ...
      io.to(roomCode).emit('domino:player-ready', {
        userId: socket.user.id, readyCount: dr.readyCount, total: dr.players.size,
        players: getPlayersInfo(dr),
      });
      const humanPlayers = Array.from(dr.players.values()).filter(p => !p.isBot);
      if (humanPlayers.every(p => p.isReady) && dr.players.size === 4) {
        await startNewRound(io, roomCode, dr, true, null);
      }
    } catch (err) { console.error('[domino:ready]', err.message); }
  });

  socket.on('domino:start-with-bots', async ({ roomCode }) => {
    try {
      console.log(`[domino] start-with-bots received from ${socket.user.username} room=${roomCode}`);
      const dr = activeDominoRooms.get(roomCode);
      if (!dr) { console.error('[domino] room not found:', roomCode); return; }
      if (dr.status !== 'waiting') { console.log('[domino] room not waiting:', dr.status); return; }
      if (!dr.players.has(socket.user.id)) {
        console.error('[domino] player not in room:', socket.user.id, 'room players:', [...dr.players.keys()]);
        return;
      }
      ...
      for (let seat = 0; seat < 4; seat++) {
        if (!takenSeats.has(seat)) {
          ...
          console.log(`[domino] Added bot at seat ${seat}`);
        }
      }

      io.to(roomCode).emit('domino:bots-added', { players: getPlayersInfo(dr) });
      // Pass socket so startNewRound emits directly to it (belt-and-suspenders)
      await startNewRound(io, roomCode, dr, true, socket);
    } catch (err) {
      console.error('[domino:start-with-bots] ERROR:', err.message, err.stack);
    }
  });
  ...
  socket.on('domino:play-tile', async ({ roomCode, tileId, side }) => {
    try {
      ...
      if (dr.currentPlayer !== socket.user.id) return socket.emit('domino:error', { message: 'No es tu turno' });
      const player = dr.players.get(socket.user.id);
      if (!player) return;
      ...
      io.to(roomCode).emit('domino:tile-played', {
        playerId: socket.user.id, tileId, side,
        board: dr.board, leftEnd: dr.leftEnd, rightEnd: dr.rightEnd,
        players: getPlayersInfo(dr),
      });
      if (player.hand.length === 0) { await endRound(io, roomCode, dr, socket.user.id, false); return; }
      advanceTurn(io, roomCode, dr);
    } catch (err) { console.error('[domino:play-tile]', err.message); }
  });
  ...
  socket.on('domino:pass', async ({ roomCode }) => {
    try {
      ...
      io.to(roomCode).emit('domino:player-passed', {
        playerId: socket.user.id, username: player.username, passCount: dr.passCount,
      });
      if (dr.passCount >= 4) { await endRound(io, roomCode, dr, null, true); return; }
      advanceTurn(io, roomCode, dr);
    } catch (err) { console.error('[domino:pass]', err.message); }
  });
  ...
  socket.on('domino:leave-room', async ({ roomCode }) => {
    try {
      ...
      await query(
        'DELETE FROM room_members WHERE room_id=(SELECT id FROM rooms WHERE code=$1) AND user_id=$2',
    ...
    } catch (err) { console.error('[domino:leave-room]', err.message); }
  });
}
