...
router.get('/', async (req, res) => {
  try {
    ...
    // Fallback query without game_type in case column doesn't exist yet
    let q, params;
    try {
      q = `SELECT r.*, u.username AS host_username,
      ...
      // game_type column may not exist yet — fallback without it
      console.warn('[rooms GET] game_type column missing, falling back:', colErr.message);
      q = `SELECT r.*, u.username AS host_username,
        (SELECT COUNT(*) FROM room_members rm WHERE rm.room_id = r.id) AS member_count
        FROM rooms r LEFT JOIN users u ON r.host_id = u.id
...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      ...
      // game_type column may not exist — insert without it
      console.warn('[rooms POST] game_type column missing, falling back:', colErr.message);
      const { rows } = await query(
        'INSERT INTO rooms (code, name, host_id, language, max_players, is_public) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
        [code, name, req.user.id, language, resolvedMax, isPublic]
      );
      room = rows[0];
    }
...
router.get('/:code', async (req, res) => {
  try {
    ...
    console.error('[rooms GET /:code]', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
