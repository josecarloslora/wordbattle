  ...
  return (
    <div className="min-h-screen p-4 max-w-5xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="flex flex-col gap-2 min-h-32">
            {loading ? <div className="flex justify-center py-8"><LoadingSpinner /></div>
              : rooms.length === 0 ? <p className="text-gray-500 text-center py-8">No rooms available</p>
        ...
        <div>
          <div className="mb-6">
            <h2 className="text-lg font-bold mb-3">Join by Code</h2>
            <div className="flex gap-2">
              <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} maxLength={6}
                placeholder="XXXXXX" className="flex-1 bg-gray-800 border border-gray-600 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 font-mono text-center text-lg tracking-widest" />
          ...
          <h2 className="text-lg font-bold mb-3">Create Room</h2>
          <form onSubmit={handleCreate} className="bg-gray-800 rounded-2xl p-4 border border-gray-700 flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <select value={createForm.maxPlayers} onChange={e => setCreateForm(f => ({ ...f, maxPlayers: parseInt(e.target.value) }))}
            ...
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setCreateForm(f => ({ ...f, isPublic: !f.isPublic }))}
                className={`w-10 h-6 rounded-full transition-colors ${createForm.isPublic ? 'bg-indigo-600' : 'bg-gray-600'}`}>
                <div className={`w-4 h-4 rounded-full bg-white mx-1 transition-transform ${createForm.isPublic ? 'translate-x-4' : 'translate-x-0'}`} />
              </button>
            </div>
