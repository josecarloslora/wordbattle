        ...
        return (
          <div key={p.id} className={`bg-gray-800 rounded-xl p-3 border ${isMe ? 'border-indigo-500' : 'border-gray-700'}`}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium truncate">{p.username} {isMe && <span className="text-indigo-400">(you)</span>}</span>
            </div>
            {p.solved ? (
              <div className="text-xs text-green-400 font-semibold">SOLVED ✓ {p.solveTime && `${p.solveTime}s`}</div>
            ) : (
                ...
                {Array(6).fill(null).map((_, row) => (
                  <div key={row} className="flex gap-0.5">
                    {Array(5).fill(null).map((_, col) => {
                      ...
                      const color = tile?.status === 'correct' ? 'bg-green-600' : tile?.status === 'present' ? 'bg-yellow-500' : tile ? 'bg-gray-600' : 'bg-gray-700';
